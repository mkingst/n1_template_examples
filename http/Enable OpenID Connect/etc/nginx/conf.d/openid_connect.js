...
function auth(r, afterSyncCheck) {
    ...
    // Get token, authz and jwks_uri from wellknown endpoint and store in keyval
    if (!r.variables["oidc_authz_endpoint"] || r.variables["oidc_authz_endpoint"] == "") {
        if (!r.variables["oidc_wellknown_endpoint"] || r.variables["oidc_wellknown_endpoint"] == "") {
            r.error("OIDC missing configuration variables: $oidc_wellknown_endpoint");
            r.return(500, r.variables.internal_error_message);
            return;
        }
        // Get authz, token and jwk_uri from wellknown endpoint
        r.subrequest("/_wellknown", function(reply) {
            if (reply.status != 200) {
                // wellknown request failed, log the reason
                var error_log = "OIDC get wellknown metadata failure";
                if (reply.status == 504) {
            ...
            // Wellknown request returned 200, check response
            try {
                var oidcMetadata = JSON.parse(reply.responseText);
                if (!oidcMetadata.authorization_endpoint || !oidcMetadata.token_endpoint || !oidcMetadata.jwks_uri) {
                    r.error("OIDC wellknown response did not include one of the following: authorization_endpoint, token_endpoint or jwks_uri");
                    if (oidcMetadata.error) {
                        r.error("OIDC get wellknown metadata error response:" + oidcMetadata.error + " " + oidcMetadata.error_description);
                    }
                    return;
                }
                // store the authz, token and jwks_uri in variables
                r.variables.oidc_authz_endpoint = oidcMetadata.authorization_endpoint;
                r.variables.oidc_token_endpoint = oidcMetadata.token_endpoint;
                r.variables.oidc_jwt_keyfile = oidcMetadata.jwks_uri;
                r.log("OIDC wellknown endpoint("+ r.variables.oidc_wellknown_endpoint+")- metadata, authorization endpoint: "+ r.variables.oidc_authz_endpoint + " token endpoint: " + r.variables.oidc_token_endpoint + " jwks_uri: " + r.variables.oidc_jwt_keyfile);
            } catch (e) {
                return;
            }
...
function getAuthZArgs(r) {
    ...
    if ( r.variables.oidc_pkce_enable == 1 ) {
        var pkce_code_verifier = c.createHmac('sha256', r.variables.oidc_hmac_key).update(String(getCryptoRand())).digest('hex');
        r.variables.pkce_id = c.createHash('sha256').update(String(getCryptoRand())).digest('base64url');
